...
public class AssistCtrl implements ActionListener, KeyListener {
	...
	/**
	 * Class constructor, sets the related frame, and the username of the
	 * current user for display in it.
	 *
	 * @param user
	 *            Name of the current user
	 * @param fr
	 *            Frame related to the controller
	 * @throws IOException
	 * @see AssistFr
	 */
	...
	/**
	 * Listens to event commands emitted by AssistFr, and reacts to them
	 * accordingly:
	 * 
	 * SEARCH: Search a patient by ID or SSN in the DB (Uses a text file as a
	 * placeholder) BACK: Go back to the previous window ID: Set the mode to ID
	 * Search (If it's not already set that way) SSN: Set the mode to SSN Search
	 * (If it's not already set that way)
	 *
	 * @param e
	 *            event triggering the action performed
	 * @see AssistFR
	 */
	@Override
	...
	/**
	 * 
	 * Listens to Keys pressed in components at AssistFr, and reacts to them
	 * accordingly:
	 * 
	...
	/**
	 * Searches for a patient by ID or SSN, depending on the active mode, and if
	 * it's found, call as method to open a new window with the patient data as
	 * argument.
	 *
	 * @throws IOException
	 * @throws SQLException
		...
		if (tf.getMode() == true) {
			...
			if (dni != null) {
				...
				if (resultado.getId() != "null") {
					...
					openPatientTecn(resultado.getName(), resultado.getLastname(), resultado.getId(), resultado.getSsn(),
						messages, name);
			...
			System.out.println("FIN MODO DNI");
			...
			System.out.println("FIN MODO SSN");
		}

	}

	/**
	 * Opens a new ASsistPatientFr to see the opened patient in as much detail
	 * as an Assistant is allowed to.
	 * 
	 * @param name
	 *            Name of the patient.
	 * @param lastname
	 ...
	 *            SSN of the patient.
	 * @param messages
	 *            Any messages related to the patient.
	 * @param user
	 *            Username of the Assistant using the program.
	 * @see AssistPatientCtrl, AssistPatientFr
	 * @throws IOException
	 */
	public void openPatientTecn(String name, String lastname, String id, String ssn, Vector<String> messages,
